...
//  using refine doesn't should error details, need to use superRefine
export const totalCostSetupFileSchema = z
	.array(z.string())
	.min(2)
	.refine(
		array => {
			...
			return deliverySetupSchema.safeParse(
				setup.trim().split(whitespaceRegex).map(Number),
			).success;
		},
		{
	...
	.refine(
		array => {
			...
			packageCosts.forEach(packageCost => {
				return packageSetupArraySchema.safeParse(
					packageCost.trim().split(whitespaceRegex),
				).success;
			});
			return true;
		},
	...
	.transform(array => {
		...
		const packageSetup = packageCosts.map(packageCost => {
			const [packageName, packageWeight, packageDistance, discountCode] =
				packageCost.trim().split(whitespaceRegex);
			return {
